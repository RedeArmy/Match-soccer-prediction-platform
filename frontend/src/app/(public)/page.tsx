...
const SCORING_RULE_META = [
  { points: 3, icon: CheckCircle2, iconColor: 'text-gold-300',   border: 'border-gold-600/30',  bg: 'bg-gold-600/[0.06]',  pill: 'bg-gold-600/20 text-gold-300'  },
  { points: 1, icon: CheckCircle2, iconColor: 'text-green-300',  border: 'border-green-500/20', bg: 'bg-green-500/[0.04]', pill: 'bg-green-500/15 text-green-300' },
  { points: 0, icon: XCircle,      iconColor: 'text-text-muted', border: 'border-white/8',      bg: 'bg-white/[0.02]',     pill: 'bg-white/8 text-text-muted'    },
]
    ...
    <div className="panel p-5 space-y-5">
      ...
      <div className="space-y-1.5">
        ...
        {leaderboardRows.map(({ pos, name, pts, you }) => (
          ...
          >
            <span className={`w-5 shrink-0 text-center text-xs font-bold tabular-nums ${you ? 'text-gold-300' : 'text-text-muted'}`}>
              {pos}
            </span>
            <span className={`flex-1 truncate ${you ? 'font-semibold text-white' : 'text-text-secondary'}`}>
              {name}
            </span>
            <span className={`font-score text-sm tabular-nums ${you ? 'text-gold-300' : 'text-text-muted'}`}>
...
export default function LandingPage() {
  ...
  const scoringRules = [
    { ...SCORING_RULE_META[0], label: t('landing.scoreExactLabel'),  example: t('landing.scoreExactEx')  },
    { ...SCORING_RULE_META[1], label: t('landing.scoreResultLabel'), example: t('landing.scoreResultEx') },
    { ...SCORING_RULE_META[2], label: t('landing.scoreMissLabel'),   example: t('landing.scoreMissEx')   },
  ]
  ...
  const techHighlights = [
    { ...TECH_HIGHLIGHT_META[0], title: t('landing.tech1Title'), desc: t('landing.tech1Desc') },
    { ...TECH_HIGHLIGHT_META[1], title: t('landing.tech2Title'), desc: t('landing.tech2Desc') },
    { ...TECH_HIGHLIGHT_META[2], title: t('landing.tech3Title'), desc: t('landing.tech3Desc') },
    { ...TECH_HIGHLIGHT_META[3], title: t('landing.tech4Title'), desc: t('landing.tech4Desc') },
  ]
  ...
  const proofPoints = [t('landing.proof1'), t('landing.proof2'), t('landing.proof3')]
  ...
  return (
    <div className="flex flex-col">
          ...
          <div className="space-y-7 pt-4">
            ...
            <div className="space-y-4">
              <h1 className="max-w-xl font-display text-6xl leading-[0.92] text-white sm:text-8xl">
                ...
                <span className="text-gold-300">{t('landing.heroGold')}</span><br />
                {t('landing.heroLine3')}
              </h1>
      ...
      <section className="px-4 py-16">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 text-center">
            <p className="text-xs uppercase tracking-widest text-gold-300">{t('landing.scoringEyebrow')}</p>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {scoringRules.map(({ points, label, example, icon: Icon, iconColor, border, bg, pill }) => (
              <div key={label} className={`card p-6 border ${border} ${bg} flex flex-col gap-4`}>
                <div className="flex items-start justify-between">
                  <Icon className={`h-6 w-6 ${iconColor}`} />
                  <span className={`rounded-full px-2.5 py-1 text-sm font-bold tabular-nums ${pill}`}>
          ...
          <div className="mt-6 flex items-start gap-3 rounded-xl border border-gold-600/20 bg-gold-600/[0.05] p-4">
            ...
            <p className="text-sm text-text-secondary">
              <span className="font-semibold text-gold-300">{t('landing.bonusLabel')}</span>{' '}
              {t('landing.bonusCopy')}
            </p>
          </div>
        </div>
      ...
      <section className="px-4 py-16 bg-white/[0.012]">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10">
            <p className="text-xs uppercase tracking-widest text-gold-300">{t('landing.formatEyebrow')}</p>
            <h2 className="mt-2 font-display text-4xl text-white sm:text-5xl">{t('landing.formatTitle')}</h2>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="card p-6 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-semibold text-white">{t('landing.groupsTitle')}</h3>
              </div>
              <p className="text-sm text-text-secondary">{t('landing.groupsDesc')}</p>
              <div className="grid grid-cols-3 gap-2 pt-1">
                {[
                  ...
                  <div key={label} className="rounded-lg border border-white/8 bg-white/[0.03] p-3 text-center">
            ...
            <div className="card p-6 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-semibold text-white">{t('landing.knockoutTitle')}</h3>
              </div>
              <p className="text-sm text-text-secondary">{t('landing.knockoutDesc')}</p>
      ...
      <section className="px-4 py-16">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 text-center">
            <p className="text-xs uppercase tracking-widest text-gold-300">{t('landing.modesEyebrow')}</p>
            <h2 className="mt-2 font-display text-4xl text-white sm:text-5xl">{t('landing.modesTitle')}</h2>
          </div>

          <div className="grid gap-6 sm:grid-cols-2 max-w-3xl mx-auto">
            {modes.map(({ name, tagLabel, tagClass, desc, features, cta, href, btnClass, cardClass, highlight }) => (
              <div key={name} className={`card p-6 border ${cardClass} flex flex-col gap-5 relative`}>
                {highlight && (
                ...
                <div className="flex items-start justify-between">
                  ...
                  <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${tagClass}`}>
                    {tagLabel}
                  </span>
                </div>
      ...
      <section className="px-4 py-16 bg-white/[0.012]">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10">
            <p className="text-xs uppercase tracking-widest text-gold-300">{t('landing.techEyebrow')}</p>
            <h2 className="mt-2 font-display text-4xl text-white sm:text-5xl">{t('landing.techTitle')}</h2>
          </div>
