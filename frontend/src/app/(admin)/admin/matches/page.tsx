  ...
  return (
    <>
      ...
      <div className="bg-white/5 rounded-lg px-4 py-3 space-y-1.5">
        ...
        <p className="text-white/50 text-xs text-center">{phaseLabel(match.phase)}</p>
        {match.kickoff_at && (
          <p className="text-white/50 text-xs text-center">{formatDateTime(match.kickoff_at)}</p>
        )}
      </div>
      ...
      <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
        ...
        <p className="text-amber-300 text-xs">
          Esto cambiará el estado del partido a <strong>en curso</strong>. Esta acción es manual y sobreescribe el ciclo automático.
        </p>
      </div>
...
export default function AdminMatchesPage() {
  ...
  const [resultForm, setResultForm] = useState<ResultForm>({ homeScore: '', awayScore: '', winMethod: '' })
  ...
  const filtered = useMemo(() => {
    ...
    if (tab === 'today') return matches.filter(m => m.kickoff_at?.slice(0, 10) === todayStr)
  ...
  const startMutation = useMutation({
    ...
    onError: (e: unknown) => {
      setModalError(e instanceof Error ? e.message : 'Error al iniciar el partido')
    },
  })
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { invalidate(); closeModal() },
    onError: (e: unknown) => {
      setModalError(e instanceof Error ? e.message : 'Error al actualizar el resultado')
    },
  })
