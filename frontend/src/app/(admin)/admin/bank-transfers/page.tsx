...
function ApproveModal({
  ...
  const overrideValid =
    !hasOverride || (!Number.isNaN(overrideQ) && overrideQ > 0);
  const amountDiffers =
    overrideCents !== null && overrideCents !== item.amount_cents;

  return (
    <>
      <ModalHeader
        title={`Aprobar Transferencia #${item.id}`}
        onClose={onClose}
      />
      ...
      <div className="space-y-1.5">
        <label
          htmlFor="approve-override"
          className="block text-sm font-medium text-white/70"
        >
          Monto a acreditar (Q){" "}
          <span className="ml-1.5 text-white/30 font-normal text-xs">
            — dejar vacío para usar el monto declarado
          </span>
        </label>
...
function RejectModal({
  ...
  return (
    <>
      <ModalHeader
        title={`Rechazar Transferencia #${item.id}`}
        onClose={onClose}
      />
...
export default function AdminBankTransfersPage() {
  ...
  const approveMutation = useMutation({
    ...
    onError: (e: unknown) => {
      setModalError(
        e instanceof Error ? e.message : "Error al aprobar la transferencia",
      );
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (e: unknown) => {
      setModalError(
        e instanceof Error ? e.message : "Error al rechazar la transferencia",
      );
    },
  });
          ...
          <div className="overflow-x-auto rounded-xl border border-white/10">
            <table className="w-full text-sm">
              ...
              <tbody>
                {paginated.map((item) => {
                  const effective =
                    item.approved_amount_cents ?? item.amount_cents;
                  const hasOverride =
