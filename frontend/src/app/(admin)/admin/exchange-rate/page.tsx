      ...
      {!isLoading && rate && (
        <div className="grid sm:grid-cols-3 gap-4">
          {[
            {
              label: "Referencia",
              value: formatRate(rate.reference_rate),
              color: "text-text-primary",
            },
            {
              label: "Compra (buy)",
              value: `Q${formatRate(rate.buy_rate)}`,
              color: "text-green-400",
            },
            {
              label: "Venta (sell)",
              value: `Q${formatRate(rate.sell_rate)}`,
              color: "text-gold-400",
            },
          ].map(({ label, value, color }) => (
            <div key={label} className="card p-5">
              <p className="text-xs text-text-muted mb-1">{label}</p>
              <p className={`font-score text-2xl font-semibold ${color}`}>
                {value}
              </p>
              <p className="text-xs text-text-muted mt-2">
                Fuente:{" "}
                <span className="text-text-secondary">{rate.source}</span>
              </p>
            </div>
          ))}
        </div>
        ...
        <button
          onClick={() => {
            if (overrideReason.length < 10) {
              setFormError("La razón debe tener al menos 10 caracteres");
              return;
            }
            setFormError("");
            override.mutate();
          ...
          disabled={
            override.isPending || !overrideRate || overrideReason.length < 10
          }
