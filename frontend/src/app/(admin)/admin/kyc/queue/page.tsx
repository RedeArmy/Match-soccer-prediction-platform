...
const statuses = ['', 'pending', 'under_review', 'escalated', 'approved', 'rejected', 'expired']
...
export default function AdminKYCQueuePage() {
  ...
  const approve = useMutation({
    mutationFn: async ({ profileID, tier }: { profileID: number; tier: number }) => {
      const token = await getToken()
          ...
          {profiles.map(p => (
            <div key={p.id} className="flex items-center justify-between gap-3 px-4 py-3">
              <div className="min-w-0 flex-1">
                ...
                {p.user_email && (
                  <p className="text-xs text-text-muted truncate">{p.user_email}</p>
                )}
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                {(p.status === 'pending' || p.status === 'under_review') && (
                  <>
                    <button
                      onClick={() => { setApproveTarget(p.id); setApproveTier(2) }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-blue-950/80 backdrop-blur-sm px-4">
          <div className="card p-6 w-full max-w-sm space-y-4">
            ...
            <div className="space-y-2">
              <p className="text-xs text-text-muted">Asignar tier de verificación</p>
              ...
              <p className="text-xs text-text-muted">
                {approveTier === 1 && 'Básico — acceso limitado a funciones de pago'}
                {approveTier === 2 && 'Estándar — límites de transacción normales'}
                {approveTier === 3 && 'Premium — sin restricciones de transacción'}
              </p>
            </div>
            <div className="flex gap-2">
              <button
              ...
              <button
                onClick={() => approve.mutate({ profileID: approveTarget, tier: approveTier })}
                disabled={approve.isPending}
