...
export default function KYCPage() {
  ...
  const [form, setForm] = useState({
    ...
    address_line: '', city: '', country: 'GT', postal_code: '',
  })
  ...
  async function handleDocUpload(e: React.ChangeEvent<HTMLInputElement>, docType: string) {
    ...
    if (!file) return
    if (file.size > 10 * 1024 * 1024) { setError('Máx. 10 MB por documento'); return }
    const mime = await sniffMIME(file)
    if (!isAllowedUploadType(mime)) { setError('Tipo no permitido. Usa JPEG, PNG, WebP o PDF.'); return }
    ...
    <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between relative">
          {statusSteps.map((step, i) => {
            ...
            const labels: Record<string, string> = {
              unverified: 'Sin verificar', submitted: 'Enviado',
              under_review: 'En revisión', approved: 'Aprobado'
            }
            return (
              <div key={step} className="flex flex-col items-center gap-1 flex-1 relative z-10">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 ${
                  done ? 'bg-gold-500 border-gold-400 text-blue-950' : 'border-blue-600 text-text-muted'
                }`}>
                  {done ? <CheckCircle2 className="w-4 h-4" /> : i + 1}
                </div>
                <span className={`text-[10px] text-center ${done ? 'text-gold-400' : 'text-text-muted'}`}>
                  {labels[step]}
                </span>
                {i < statusSteps.length - 1 && (
                  <div className={`absolute top-4 left-1/2 w-full h-0.5 -z-10 ${done && i < currentStep ? 'bg-gold-500' : 'bg-blue-700'}`} />
                )}
              </div>
            )
          })}
      ...
      {(!kyc?.status || kyc.status === 'unverified' || kyc.status === 'rejected') && (
        <div className="card p-6 space-y-4">
          ...
          >
            {submitProfile.isPending ? <LoadingSpinner size={18} /> : 'Enviar información'}
          </button>
        </div>
      )}
      ...
      {kyc?.status && kyc.status !== 'unverified' && (
        <div className="card p-6 space-y-4">
          ...
          <p className="text-xs text-text-muted">JPEG, PNG, WebP o PDF — máx. 10 MB por archivo</p>
