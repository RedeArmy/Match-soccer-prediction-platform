...
export default function WithdrawPage() {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const extra: Record<string, unknown> = method === 'bank_gt'
    ...
    onSuccess: () => {
      setSuccess('Solicitud de retiro enviada. Será procesada en 1-3 días hábiles.')
      setAmount('')
    },
    ...
    return (
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <div className="card p-8 flex flex-col items-center text-center gap-4">
          ...
          <div>
            <h2 className="text-lg font-semibold text-white mb-2">Verificación requerida</h2>
            <p className="text-text-secondary text-sm">
              Debes completar tu verificación KYC (Tier 2) para poder retirar fondos.
            </p>
          </div>
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      ...
      <div className="card p-4 flex items-center justify-between">
        <span className="text-sm text-text-secondary">Disponible para retirar</span>
      ...
      <div className="card p-6 space-y-5">
        ...
        <div>
          ...
          {amountCents > available && (
            <p className="text-red-400 text-xs mt-1">Excede el balance disponible</p>
          )}
        </div>
        ...
        {method === 'bank_gt' && (
          <>
            {[
              { value: bankName,       setter: setBankName,       label: 'Nombre del banco',   placeholder: 'Ej. Banco Industrial' },
              { value: accountNumber,  setter: setAccountNumber,  label: 'Número de cuenta',   placeholder: '000-000000-0' },
              { value: accountHolder,  setter: setAccountHolder,  label: 'Titular de la cuenta', placeholder: 'Nombre completo' },
            ].map(({ value, setter, label, placeholder }) => (
              <div key={label}>
                <label className="block text-sm text-text-secondary mb-1.5">{label}</label>
                <input type="text" value={value} onChange={e => setter(e.target.value)} placeholder={placeholder} className="input-base" />
              </div>
            ))}
          </>
        )}
        ...
        >
          {mutation.isPending ? <LoadingSpinner size={18} /> : `Retirar ${formatGTQ(amountCents)}`}
        </button>
