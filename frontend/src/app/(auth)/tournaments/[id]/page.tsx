...
import { ArrowLeft, Check, Copy, Crown, Loader2, LogOut, Medal, Settings, Trophy, UserCheck, UserX, Users, X } from 'lucide-react'
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {!group?.is_premium && (
          <div className="card flex items-center gap-3 p-4">
            ...
            <div>
              <p className="text-lg font-semibold text-text-muted">{t('status.free')}</p>
            </div>
          </div>
        )}
        ...
        {group?.mode_general && (
          <div className="card flex items-center gap-3 p-4">
            ...
            <div>
              ...
              <p className="text-xs text-text-muted">{t('group.prizePoolGeneral')}</p>
            </div>
          </div>
        )}
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        ...
        <section className="panel p-4 sm:p-5">
          ...
          {entries.length > 0 && (
            <LeaderboardTable entries={entries} t={t} />
          )}
        </section>
          ...
          {entries.map((entry) => (
            ...
            >
              ...
              <td className="max-w-[140px] px-2 py-2.5">
                <span className={cn('block truncate', entry.prize_winner ? 'font-semibold text-gold-200' : 'text-text-primary')}>
                  {entry.user_name}
                </span>
              </td>
              {/* Per-round points */}
              {activeRounds.map((rd) => {
                ...
                return (
                  <td key={rd.key} className="px-2 py-2.5 text-center">
                    <span className={cn('text-xs tabular-nums', pts > 0 ? 'text-white' : 'text-text-muted/40')}>
                      {pts > 0 ? pts : '—'}
                    </span>
                  </td>
                )
