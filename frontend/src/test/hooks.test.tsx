...
describe('useExchangeRate', () => {
  ...
  it('returns data when getExchangeRate resolves', async () => {
    const rate = { buy_rate: '7.72', sell_rate: '7.80', effective_at: '2026-01-01T00:00:00Z', stale: false }
    vi.mocked(api.getExchangeRate).mockResolvedValueOnce(rate)

    const { result } = renderHook(() => useExchangeRate(), { wrapper: makeWrapper() })
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
    expect(result.current.data).toEqual(rate)
  })

  it('has no data when getExchangeRate rejects', async () => {
    vi.mocked(api.getExchangeRate).mockRejectedValueOnce(new Error('network error'))
...
describe('useBalance', () => {
  beforeEach(() => {
    ...
    vi.mocked(useAuth).mockReturnValue({ getToken: vi.fn().mockResolvedValue('tok_abc') } as never)
  })
...
describe('useKYCStatus', () => {
  ...
  it('returns KYC data when getKYCStatus resolves', async () => {
    const kyc = { id: 1, user_id: 'u1', status: 'approved', tier: 1, created_at: '', updated_at: '' }
    vi.mocked(api.getKYCStatus).mockResolvedValueOnce(kyc as never)

    const { result } = renderHook(() => useKYCStatus(), { wrapper: makeWrapper() })
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
