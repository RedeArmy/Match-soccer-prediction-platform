...
function renderDashboard() {
  return render(<I18nProvider><DashboardContent /></I18nProvider>)
}
...
describe('DashboardContent – groups section', () => {
  ...
  it('renders a card per group when groups exist', () => {
    setQueries({ groups: [mockGroup, { ...mockGroup, id: 2, name: 'Segunda Kiniela' }], ledger: [] })
    renderDashboard()
...
describe('DashboardContent – transactions section', () => {
  ...
  it('renders withdrawal_release as refund', () => {
    setQueries({
      groups: [],
      ledger: [{ ...mockLedgerEntry, kind: 'withdrawal_release', delta_cents: 3000 }],
    })
    renderDashboard()
    expect(screen.getByText(/\+/)).toBeInTheDocument()
  })
...
describe('DashboardContent – GroupCard pending membership', () => {
  ...
  it('shows pending badge for a pending-membership group', () => {
    const pendingGroup: GroupResponse = { ...mockGroup, membership_status: 'pending' }
  ...
  it('renders view-all link for an active group', () => {
    ...
    const tournamentLink = links.find(l => l.getAttribute('href')?.startsWith('/tournaments/'))
