...
const userRow = {
  ...
  predictions: [{ match_id: 5, home_score: 2, away_score: 1, has_prediction: true }],
}

function mockQueryWith(data: LivePredictionsResponse | undefined, isLoading = false) {
...
describe('LivePredictionsCarousel', () => {
  ...
  it('shows empty message when live matches exist but no user predictions', () => {
    mockQueryWith({ live_matches: [liveMatch], user_predictions: [] })
    renderCarousel()
    expect(screen.getByText('No hay partidos en vivo en este momento.')).toBeTruthy()
  })
  ...
  it('shows noPrediction label when user has no submitted score', () => {
    const noScoreRow = {
      ...userRow,
      predictions: [{ match_id: 5, home_score: 0, away_score: 0, has_prediction: false }],
    }
    mockQueryWith({ live_matches: [liveMatch], user_predictions: [noScoreRow] })
    renderCarousel()
  ...
  it('accordion opens and shows match scores on click', () => {
    ...
    const toggle = screen.getByRole('button', { name: /Ver predicciones de Alice Doe/i })
  ...
  it('shows multiple user cards', () => {
    ...
    mockQueryWith({ live_matches: [liveMatch], user_predictions: [userRow, bobRow] })
    renderCarousel()
