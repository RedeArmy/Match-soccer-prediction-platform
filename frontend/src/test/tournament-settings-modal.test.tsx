...
vi.mock('@tanstack/react-query', () => ({
  useMutation:     vi.fn(),
  useQueryClient:  vi.fn().mockReturnValue({ setQueryData: vi.fn(), invalidateQueries: vi.fn() }),
}))
...
function makeGroup(overrides: Partial<GroupDetailResponse> = {}): GroupDetailResponse {
  return {
    id: 1,
    name: 'Test Pool',
    owner_user_id: 1,
...
function renderModal(props: Partial<Parameters<typeof TournamentSettingsModal>[0]> = {}) {
  const defaults = {
    group: makeGroup(),
    memberCount: 3,
    onClose: vi.fn(),
...
describe('TournamentSettingsModal – free group, enough members (canUpgrade)', () => {
  ...
  it('toggling mode_general checkbox changes its checked state', () => {
    renderModal({ group: makeGroup({ is_premium: false, mode_general: false }), memberCount: 6 })
...
describe('TournamentSettingsModal – premium group', () => {
  ...
  it('does not show any eligibility banner', () => {
    renderModal({ group: makeGroup({ is_premium: true, mode_general: true, mode_round: false }) })
...
describe('TournamentSettingsModal – mutation states', () => {
  it('shows spinner when mutation is pending', () => {
    vi.mocked(useMutation).mockReturnValue({
      mutate: vi.fn(), isPending: true, isError: false, error: null,
    } as never)
    const { container } = renderModal({ group: makeGroup({ is_premium: true }), memberCount: 6 })
    expect(container.querySelector('.animate-spin')).toBeInTheDocument()
  })

  it('shows error message when mutation fails', () => {
    vi.mocked(useMutation).mockReturnValue({
      mutate: vi.fn(), isPending: false, isError: true, error: { message: 'Server error' },
    } as never)
  ...
  it('shows fallback "Error" when error has no message', () => {
    vi.mocked(useMutation).mockReturnValue({
      mutate: vi.fn(), isPending: false, isError: true, error: {},
    } as never)
    renderModal({ group: makeGroup({ is_premium: true }), memberCount: 6 })
