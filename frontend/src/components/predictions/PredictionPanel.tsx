...
import { CalendarClock, CheckCircle2, MapPin, Save, SlidersHorizontal, Target } from 'lucide-react'
...
export function PredictionPanel() {
  ...
  const mutation = useMutation({
    mutationFn: async ({ match, draft }: { match: MatchResponse; draft: { home: number; away: number } }) => {
      const token = await getToken()
      const existing = predictionByMatch.get(match.id)
      if (existing) {
        return api.updatePrediction(token!, existing.id, { home_score: draft.home, away_score: draft.away })
      }
      return api.submitPrediction(token!, { match_id: match.id, home_score: draft.home, away_score: draft.away })
    },
    onSuccess: async () => {
  ...
  const sortedMatches = useMemo(() => {
    return [...(matchesQuery.data ?? [])].sort((a, b) => new Date(a.starts_at).getTime() - new Date(b.starts_at).getTime())
  }, [matchesQuery.data])
  ...
  return (
    <section className="panel p-4 sm:p-5">
      <div className="mb-5 flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
        <div>
          <div className="mb-2 flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-white">{t('predictions.title')}</h2>
          </div>
          <p className="max-w-2xl text-sm text-text-secondary">{t('predictions.subtitle')}</p>
        </div>
          ...
          {([
            ...
            <button
              ...
              className={cn(
                ...
                filter === key ? 'bg-gold-400 text-blue-950' : 'text-text-muted hover:text-text-primary',
              )}
            >
              {label}
            </button>
      ...
      {!isLoading && visibleMatches.length > 0 && (
        <div className="grid gap-3">
          {visibleMatches.map((match) => {
            ...
            const pending = mutation.isPending && mutation.variables?.match.id === match.id

            return (
              <article key={match.id} className="rounded border border-white/10 bg-white/[0.025] p-4">
                <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
                  <div className="min-w-0">
                    <div className="mb-3 flex flex-wrap items-center gap-2">
                      ...
                      <span className={cn(
                        ...
                        prediction ? 'border-green-400/30 bg-green-400/10 text-green-200' : 'border-gold-400/25 bg-gold-400/10 text-gold-200',
                      )}>
                        {prediction ? <CheckCircle2 className="h-3 w-3" /> : <Target className="h-3 w-3" />}
                        {prediction ? t('predictions.saved') : t('predictions.unsaved')}
                      </span>
                      {locked && <span className="text-[10px] uppercase text-red-300">{t('predictions.locked')}</span>}
                    </div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-text-muted">
                      <span className="inline-flex items-center gap-1.5">
                        ...
                        {t('predictions.kickoff')}: {formatDateTime(match.starts_at)}
                      </span>
                      {match.stadium && (
                        <span className="inline-flex items-center gap-1.5">
                      ...
                      {(match.phase || match.group_label) && (
                        <span>{t('predictions.phase')}: {match.phase ?? match.group_label}</span>
                      )}
                      {prediction?.points !== null && prediction?.points !== undefined && (
                        <span className="text-gold-300">{prediction.points} {t('predictions.points')}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
                    <ScoreInput
                      ...
                      value={draft.home}
                      disabled={locked}
                      onChange={(value) => setDrafts((current) => ({ ...current, [match.id]: { ...draft, home: value } }))}
                    />
                    <ScoreInput
                      label={t('predictions.away')}
                      value={draft.away}
                      disabled={locked}
                      onChange={(value) => setDrafts((current) => ({ ...current, [match.id]: { ...draft, away: value } }))}
                    />
                    <button
                      type="button"
                    ...
                    >
                      ...
                      {pending ? t('common.saving') : prediction ? t('predictions.update') : t('predictions.submit')}
                    </button>
                  </div>
                </div>
              </article>
      ...
      <p className="mt-4 text-xs text-text-muted">{t('predictions.exactHint')}</p>
    </section>
  )
}

function TeamLabel({ label, align }: Readonly<{ label: string; align: 'left' | 'right' }>) {
  return (
...
function ScoreInput({ label, value, disabled, onChange }: Readonly<{
  label: string
  value: number
  disabled: boolean
  ...
  return (
    <label className="block w-full sm:w-20">
      <span className="mb-1 block text-[10px] uppercase text-text-muted">{label}</span>
      <input
        type="number"
        min={0}
        max={30}
