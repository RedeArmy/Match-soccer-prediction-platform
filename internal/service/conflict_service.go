...
func (s *conflictService) ListConflicts(ctx context.Context) ([]domain.Conflict, error) {
	...
	stalePayments, err := s.paymentRepo.ListStale(ctx, staleThreshold)
	if err != nil {
	...
	// Stale pending memberships
	staleMemberships, err := s.memberRepo.ListStalePending(ctx, staleThreshold)
	if err != nil {
