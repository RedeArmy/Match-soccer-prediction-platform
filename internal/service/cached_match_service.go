...
func (s *cachedMatchService) UpdateResult(ctx context.Context, id int, score ScoreUpdate) (*domain.Match, error) {
	m, err := s.inner.UpdateResult(ctx, id, score)
	if err != nil {
		return nil, err
	}
...
func (s *cachedMatchService) CorrectResult(ctx context.Context, id int, score ScoreUpdate) (*domain.Match, error) {
	m, err := s.inner.CorrectResult(ctx, id, score)
	if err != nil {
		return nil, err
	}
	s.invalidateMatchLists(ctx, m.Phase, m.Status)
