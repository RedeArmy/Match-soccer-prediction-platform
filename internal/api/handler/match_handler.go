...
func (h *MatchHandler) UpdateResult(w http.ResponseWriter, r *http.Request) {
	id, homeScore, awayScore, winMethod, penaltyWinner, penaltyHomeScore, penaltyAwayScore, ok := h.parseScoreChange(w, r)
	if !ok {
		return
	}
	match, err := h.svc.UpdateResult(r.Context(), id, homeScore, awayScore, winMethod, penaltyWinner, penaltyHomeScore, penaltyAwayScore)
	if err != nil {
		writeError(w, r, h.log, err)
		return
	}
...
func (h *MatchHandler) CorrectMatchResult(w http.ResponseWriter, r *http.Request) {
	...
	match, err := h.svc.CorrectResult(r.Context(), id, homeScore, awayScore, winMethod, penaltyWinner, penaltyHomeScore, penaltyAwayScore)
	if err != nil {
		writeError(w, r, h.log, err)
		return
	}
...
func (h *MatchHandler) parseScoreChange(w http.ResponseWriter, r *http.Request) (id, homeScore, awayScore int, winMethod *domain.WinMethod, penaltyWinner *string, penaltyHomeScore, penaltyAwayScore *int, ok bool) {
	id, err := pathID(r, "id")
	if err != nil {
		writeError(w, r, h.log, err)
		return
	...
	return id, *req.HomeScore, *req.AwayScore, winMethod, req.PenaltyWinner, req.PenaltyHomeScore, req.PenaltyAwayScore, true
}
