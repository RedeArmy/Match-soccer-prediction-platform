...
func TestPushDigestGate_P2_UnderThreshold_SendsIndividual(t *testing.T) {
	...
	for i := range 5 {
		send, count := g.Record(1, notification.PriorityP2Medium, now)
		if !send {
...
func TestPushDigestGate_P2_FirstOverflow_SendsDigest(t *testing.T) {
	...
	for range 5 {
		g.Record(1, notification.PriorityP2Medium, now) //nolint:errcheck
	}
...
func TestPushDigestGate_WindowExpiry_ResetsCount(t *testing.T) {
	...
	// exhaust the window
	for range 3 {
		g.Record(1, notification.PriorityP2Medium, now)
	}

	// advance past the window
	future := now.Add(2 * time.Second)
	send, count := g.Record(1, notification.PriorityP2Medium, future)
	if !send {
...
func TestPushDigestGate_IndependentUsersDoNotInterfere(t *testing.T) {
	...
	send, count := g.Record(2, notification.PriorityP2Medium, now)
	if !send {
...
func TestPushDigestGate_Prune_RemovesExpiredWindows(t *testing.T) {
	...
	g.Record(1, notification.PriorityP2Medium, now)
	g.Record(2, notification.PriorityP2Medium, now)
	...
	send1, _ := g.Record(1, notification.PriorityP2Medium, now.Add(2*time.Second))
	send2, _ := g.Record(2, notification.PriorityP2Medium, now.Add(2*time.Second))
	if !send1 || !send2 {
...
func TestPushDigestGate_P3Low_GatedLikeP2(t *testing.T) {
	...
	send, _ := g.Record(99, notification.PriorityP3Low, now)
	if !send {
		t.Error("first P3 push: expected sendIndividual=true")
	}

	// Second P3 push: digest
	send, count := g.Record(99, notification.PriorityP3Low, now)
	if send {
