...
func (r *PostgresKYCProfileRepository) ListPending(ctx context.Context, f KYCProfileFilters, p Pagination) ([]*domain.KYCProfile, error) {
	...
	// Prefix every column with the table alias so the WHERE clause
	// generated by whereBuilder (which uses bare column names) still works.
	const joinCols = `
		kp.id, kp.user_id, kp.status, kp.tier,
		kp.full_name, kp.date_of_birth, kp.nationality,
		kp.document_type, kp.document_number,
		kp.address_line, kp.city, kp.country, kp.postal_code,
		kp.submitted_at, kp.reviewed_at, kp.reviewed_by, kp.rejection_reason,
		kp.risk_score, kp.pep_flag, kp.sanctions_flag,
		kp.balance_frozen, kp.frozen_amount_cents, kp.frozen_reason,
		kp.next_review_at, kp.submission_ip::text, kp.created_at, kp.updated_at,
		u.email`
	base := `SELECT` + joinCols + ` FROM kyc_profiles kp JOIN users u ON u.id = kp.user_id AND u.deleted_at IS NULL`
	q := base + wb.clause() + " ORDER BY kp.submitted_at ASC NULLS LAST"
	...
	return collectRows(rows, func(r pgx.Rows) (*domain.KYCProfile, error) {
		return scanKYCProfileWithEmail(r)
	})
}
