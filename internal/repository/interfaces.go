...
type PaymentIntentRepository interface {
	...
	// that are still in status='pending' and were created strictly before before.
	// Used by the worker to enforce the 1-hour auto-expiry policy for online
	// card/wallet checkout sessions (both providers expire sessions after ~1 hour).
	// Bank transfers are stored in a separate table and are unaffected.
	// Returns the number of rows updated.
	CancelStalePendingCardIntents(ctx context.Context, before time.Time) (int64, error)
}
